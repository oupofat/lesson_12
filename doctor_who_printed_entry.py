...
Record (episode = "The Doctor Falls", season = 1, rating = 5.29, doctor = "Peter Capaldi", writer = "Steven Moffat"),
Record (episode = "Rise of the Cybermen", season = 1, rating = 9.22, doctor = "David Tennant", writer = "Tom MacRae"),
Record (episode = "The Runaway Bride", season = 1, rating = 9.35, doctor = "David Tennat", writer = "Russel T Davis"),
...
#2. Find the highest rated episode per season. Return a dictionary with
#the season number as the key and the episode record as the value.
highest_rated = {}
for record in doctor_who:
    if record.rating not in highest_rated:
        highest_rated[record.season]= record.episode 
    elif record.rating < highest_rated[record.season].rating:
        highest_rated[record.season]=record.episode
for entry in highest_rated.items():
    print("Season",entry[0],"had this episode",entry[1],)